...
void Renderer::InitFrameBuffer()
{
	...
	for (int fo = 0; fo < 3; ++fo)
	{
		...
		f->glGenTextures(1, m_colorTex[fo]);
		...
		// attach colors
		for (int i = 0; i < 1; ++i) {
			f->glFramebufferTexture(GL_DRAW_FRAMEBUFFER,
				GL_COLOR_ATTACHMENT0 + i, m_colorTex[fo][i], 0);
		}
		// attach color
...
void Renderer::InitObjects()
{
	...
	m_screenQuad4->SetType(1);
	m_screenQuad4->SetFlipV(1);
	m_screenQuad4->SetColorAttachment(m_depthTex[0]);
...
void Renderer::DrawAll(GLuint defFrameBuffer)
{
	...
	//for (auto obj : m_objects)
	//{
	//	obj->Draw(m_mvp, m_camera->camera_position);
	//}
	...
	auto cmp = m_camera->camera_pitch;
	auto cmps = m_camera->camera_position;

	//m_camera->camera_pitch = -m_camera->camera_pitch;
	//m_camera->camera_position.y = -m_camera->camera_position.y;
	//m_camera->Update();
	//auto cm_pos = m_camera->camera_position;
	//cm_pos.y = -cm_pos.y;
	//auto new_view = glm::lookAt(cm_pos, m_camera->camera_look_at, m_camera->camera_up);
